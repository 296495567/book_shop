...
class MemberController extends Controller
{
  public function register(Request $request)
  {
    ...
    // 邮箱注册
      ...
      $member = new Member;
      $email_user = Member::where('email',$email)->first();
      if($email_user != ""){
          $m3_result->status = 10;
          $m3_result->message = '该邮箱已经注册过';
      }else{
        $member->email = $email;
        $member->password = md5('bk' + $password);
        $member->save();
        ...
        $m3_email->to = $email;
        // $m3_email->cc = '296495567@qq.com';
        $m3_email->subject = 'King书店验证';
        $m3_email->content = '请于24小时点击该链接完成验证. '.$_SERVER['SERVER_NAME'].'/service/validate_email'
                          . '?member_id=' . $member->id
                          . '&code=' . $uuid;
